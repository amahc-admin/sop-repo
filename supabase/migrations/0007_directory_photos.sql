-- Lets each directory entry carry a photo, uploaded straight from the
-- browser to Supabase Storage (no external hosting needed). Like
-- reports_to before it, this travels inside the existing p_entry/
-- p_fields jsonb blobs -- no RPC signature changes.
--
-- Storage note: this bucket is public-read and open to anon uploads,
-- matching the rest of this app's security model -- the shared passcode
-- gates the actual directory_entries write (name/role/etc, via the
-- RPCs below), not the raw file upload itself. Anyone with the site's
-- public anon key (which is meant to be public, same as every other API
-- call here) could technically upload directly to this bucket; that's
-- the same trust level as every other write path in this app.

insert into storage.buckets (id, name, public, file_size_limit, allowed_mime_types)
values ('directory-photos', 'directory-photos', true, 5242880, array['image/jpeg', 'image/png', 'image/webp', 'image/gif'])
on conflict (id) do nothing;

drop policy if exists "directory photos are publicly readable" on storage.objects;
create policy "directory photos are publicly readable"
  on storage.objects for select
  using (bucket_id = 'directory-photos');

drop policy if exists "anyone can upload a directory photo" on storage.objects;
create policy "anyone can upload a directory photo"
  on storage.objects for insert
  with check (bucket_id = 'directory-photos');

alter table directory_entries add column if not exists photo_url text;
...
begin
  ...
  insert into directory_entries (id, name, role, department, email, phone, notes, reports_to, photo_url, status)
  values (
    v_id, v_name, p_entry->>'role', p_entry->>'department',
    nullif(p_entry->>'email', ''), nullif(p_entry->>'phone', ''), nullif(p_entry->>'notes', ''),
    v_reports_to, nullif(p_entry->>'photo_url', ''), 'draft'
  );
...
begin
  ...
  update directory_entries set
    ...
    photo_url = case when p_fields ? 'photo_url' then nullif(p_fields->>'photo_url', '') else photo_url end,
...
begin
  ...
  update directory_entries set
    ...
    photo_url = case when v_proposal.fields ? 'photo_url' then nullif(v_proposal.fields->>'photo_url', '') else photo_url end,

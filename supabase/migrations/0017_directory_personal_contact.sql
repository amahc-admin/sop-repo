-- Lets a Directory entry carry a separate personal email/phone
-- alongside their existing (company) email/phone -- same jsonb-blob
-- extension pattern as reports_to/photo_url before it, no RPC
-- signature changes. The existing email/phone columns are unchanged
-- and now read as "company" contact info in the UI.

alter table directory_entries add column if not exists personal_email text;
alter table directory_entries add column if not exists personal_phone text;
...
begin
  ...
  insert into directory_entries (id, name, role, department, email, phone, personal_email, personal_phone, notes, reports_to, photo_url, status)
  values (
    v_id, v_name, p_entry->>'role', p_entry->>'department',
    nullif(p_entry->>'email', ''), nullif(p_entry->>'phone', ''),
    nullif(p_entry->>'personal_email', ''), nullif(p_entry->>'personal_phone', ''),
...
begin
  ...
  update directory_entries set
    ...
    personal_email = case when p_fields ? 'personal_email' then nullif(p_fields->>'personal_email', '') else personal_email end,
    personal_phone = case when p_fields ? 'personal_phone' then nullif(p_fields->>'personal_phone', '') else personal_phone end,
...
begin
  ...
  update directory_entries set
    ...
    personal_email = case when v_proposal.fields ? 'personal_email' then nullif(v_proposal.fields->>'personal_email', '') else personal_email end,
    personal_phone = case when v_proposal.fields ? 'personal_phone' then nullif(v_proposal.fields->>'personal_phone', '') else personal_phone end,

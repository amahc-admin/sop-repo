-- Lets the Edit SOP form move an existing SOP to a different department
-- -- previously the department was only ever set once, at creation
-- (add_sop/add_sop_document), with no way to change it afterward. Same
-- jsonb-extension pattern as every other field here: p_fields->>'department'
-- flows straight through from the Edit form's new dropdown, for both the
-- Admin-direct path (edit_sop) and the Member-proposal path (already
-- stored as-is by propose_sop_edit -- only applying it in approve_sop_edit
-- needs to change).
...
begin
  ...
  if p_fields ? 'department' and not exists (select 1 from departments where id = p_fields->>'department') then
...
begin
  ...
  if v_proposal.fields ? 'department' and not exists (select 1 from departments where id = v_proposal.fields->>'department') then
    raise exception 'unknown department';
  end if;

...
}
{
  ...
  // --- Cave Handbook patch (see web/vendor/pdfjs/README.md) ---
  // This viewer is self-hosted on the handbook's own GitHub Pages
  // origin, not Mozilla's, so the same-origin check below would
  // otherwise reject every real file (every uploaded SOP document
  // lives in Supabase Storage, a different origin). Explicitly trust
  // that one known origin instead of disabling the check entirely.
  const TRUSTED_FILE_ORIGINS = ["https://ekavaiissbgxcyvtorjz.supabase.co"];
  var validateFileURL = function (file) {
    ...
    try {
      ...
      if (fileOrigin !== viewerOrigin && !TRUSTED_FILE_ORIGINS.includes(fileOrigin)) {
